...
# Configuración de extracción Serie P4 (Universo PSCV)
# Se extraen las filas correspondientes a la sección de existencia de patologías y factores de riesgo
CONFIG_EXTRACCION = [
    {"fila": "16", "label": "Hipertensión arterial"},
    {"fila": "17", "label": "Diabetes Mellitus tipo 2"},
    {"fila": "18", "label": "Dislipidemia"},
    {"fila": "19", "label": "Tabaquismo >=55 años"},
    {"fila": "20", "label": "Antecedentes de infarto agudo al miocardio (IAM)"},
    {"fila": "21", "label": "Antecedentes de ataque cerebro vascular (ACV)"},
    {"fila": "22", "label": "Antecedentes de otras enfermedades Cardiovasculares ECV"},
    {"fila": "23", "label": "Enfermedad Renal crónica (etapa 1 a 5)"}
]

# Grupos etarios estandarizados para la Serie P (Rango F:AG en Hoja P4)
GRUPOS_ETARIOS = [
...
print(f"--- Iniciando Extracción Masiva Universo PSCV ({ANIO}) ---")
    ...
    for _, centro in df_centros.iterrows():
        ...
        if not os.path.exists(ruta_archivo):
            ...
            if os.path.exists(ruta_archivo_alt):
                ...
                print(f"Falta archivo Serie P: {archivo_nombre}")
            ...
            for config in CONFIG_EXTRACCION:
                ...
                for i, grupo in enumerate(GRUPOS_ETARIOS):
                    ...
                    meta_base = {
                        ...
                        "Serie": "P",
                        "Hoja": "P4",
                        "Patologia/Factor Riesgo": patologia_label,
                        "Grupo Etario": grupo
                    }
...
if datos_maestros:
    ...
    ruta_xlsx = os.path.join(RUTA_SALIDA_CARPETA, "p_universo_PSCV.xlsx")
    df_final.to_excel(ruta_xlsx, index=False, sheet_name="PSCV")
